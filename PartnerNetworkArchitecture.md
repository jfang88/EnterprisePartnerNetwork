...
flowchart TB
    ...
    B2 -->|"Delegates to GTM 4"| C1
    B2 -->|"Delegates to GTM 5"| D1

    %% DNS → GTM site priority (6–9)
    C1 -->|"Local DNS zone (GTM) 6"| C1
    C1 -->|"Topology / site priority 7"| C3
    D1 -->|"Local DNS zone (GTM) 8"| D1
    D1 -->|"Topology / site priority 9"| D3

    %% LTM → WAF → Private Cloud (10–14)
    C3 -->|"HTTP(S) traffic 10"| C5
    C3 -->|"Health/management 11"| C4
    C5 -->|"Allowed traffic 12"| E2
    E2 -->|"To apps A1, A2 13"| E3
    E2 -->|"To apps A1, A2 14"| E4

    D3 -->|"HTTP(S) traffic 15"| D5
    D3 -->|"Health/management 16"| D4
    D5 -->|"Allowed traffic 17"| F2
    F2 -->|"To apps B1, B2 18"| F3
    F2 -->|"To apps B1, B2 19"| F4

    %% Cross‑site / loopback DNS (20–23)
    E3 -->|"May resolve via GTM 20"| C1
    F3 -->|"May resolve via GTM 21"| D1
    C1 -->|"Loopback to local WAF 22"| C5
    D1 -->|"Loopback to local WAF 23"| D5
```

You can paste this Mermaid diagram into any Mermaid‑capable tool (e.g., Git‑based wiki, VS Code + Mermaid plugin) and export an image for inclusion in a design deck.
...
- Internal clients via corporate DNS rely on **one delegation point** to GTM; **your GTM site‑priority** drives failover.  
- Direct‑to‑GTM‑DNS clients may see **some round‑robin over DNS IPs**; consider DNS‑view or topology rules if strict site‑local DNS is needed.  
...
These can be filled in later by:

- **Network / F5 team** (GTM / LTM / WAF config).  
- **DNS / IAM team** (corporate DNS design).  
- **Application teams** (session model and user‑experience on failover).  

---

## 7. Prompt file for regeneration

Copy this block into a separate `.md` file (e.g., `prompt-extranet-f5
